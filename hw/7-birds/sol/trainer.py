...
class Trainer:
    ...
    def process_batch(self, batch, is_train: bool, metrics: MetricTracker):
        ...
        batch["labels"] = torch.argmax(batch["pred"], dim=1).detach()
    ...
    def _log_image(self, img, **kwargs):
        cur_img = img[np.random.choice(np.arange(len(img)))]
        self.writer.add_image('image', np.transpose(cur_img.detach().cpu().numpy(), axes=(1, 2, 0)))
